...
test('parseStages: 単体ステージをそのまま返す', () => {
  ...
  assert.deepEqual(parseStages('m3b'), ['m3b']);
  assert.deepEqual(parseStages('m5'), ['m5']);
  assert.deepEqual(parseStages('m7'), ['m7']);
});

test('parseStages: 範囲指定を展開する（m3bはm3とm4の間に位置する）', () => {
  assert.deepEqual(parseStages('m2-m4'), ['m2', 'm3', 'm3b', 'm4']);
  assert.deepEqual(parseStages('m1-m7'), ['m1', 'm2', 'm3', 'm3b', 'm4', 'm5', 'm6', 'm7']);
});
